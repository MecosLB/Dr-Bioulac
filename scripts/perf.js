new PerformanceObserver((list) => {
    ...
    const doctTTFB = navEntry.responseStart;

    const lcpRequestStart = Math.max(doctTTFB, lcpResEntry ? lcpResEntry.requestStart : 0);
    ...
    console.log('document_ttfb', doctTTFB);
    console.log('resource_load_delay', lcpRequestStart - doctTTFB);
    ...
    performance.measure('document_ttfb',
        { start: 0, end: doctTTFB });
    performance.measure('resource_load_delay',
        { start: doctTTFB - 0.01, end: lcpRequestStart - 0.01 });
